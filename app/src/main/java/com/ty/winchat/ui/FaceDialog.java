...
//		}
		...
		int mod = imageIds.length % 21;
		int pageNo = imageIds.length / 21 + (mod > 0 ? 1 : 0);
		for(int i=0;i<pageNo;i++){
			GridView view = new GridView(context);
			SimpleAdapter simpleAdapter;
			int start = i * 21;
			int end = (i+1) * 21;
			if (end > listItems.size()) end = listItems.size();
			simpleAdapter= new SimpleAdapter(context, listItems.subList(start, end), R.layout.face_dialog_gridview_item, new String[]{"image"}, new int[]{R.id.image});
