...
//		 SpotManager.getInstance(this).showSpotAds(this);
		...
		public void run() {
			...
			try {
				...
				while (socket.isConnected()) {
                    ...
                    //1：将YUV数据格式转化成jpeg
					YuvImage image = queue.take();
                    if(!image.compressToJpeg(rect, 100, os))  continue ;
    ...
    //		            matrix.postScale(2.0f, 2.0f);
					...
					out.write(Util.intToBytes(baos.size()));
...
//				socket.close();
	...
	BlockingQueue<YuvImage> queue = new LinkedBlockingDeque<YuvImage>();
...
//		            out.write(baos.toByteArray());
//		            out.flush();
//					System.out.println("Send one bitmap to endpoint");
//		            out.close();
//		            socket.close();
//				}  catch (IOException e) {
//					e.printStackTrace();
